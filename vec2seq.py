...
class EncoderDecoderModel:
    ...
    def __make_model(self):
        self.__model = wrapper.make_model(
            ...
            w_x = functions.Linear(self.__src_size, 4 * self.__n_hidden),
            # decoder
    ...
    @staticmethod
    def new(src_size, trg_vocab, n_embed, n_hidden):
        self = EncoderDecoderModel()
        self.__src_size = src_size
    ...
    def save(self, filename):
        with ModelFile(filename, 'w') as fp:
            ...
            fp.write_linear(self.__model.w_x)
    ...
    def load(filename):
        ...
        with ModelFile(filename) as fp:
            ...
            fp.read_linear(self.__model.w_x)
...
def parse_args():
    ...
    p.add_argument('source', help='[in] source vectors, one per line')
...
def read_src_vectors(filename):
    vectors = []
    with open(filename) as fp:
        for line in fp:
            vectors.append(np.array([float(x) for x in line.strip().split()], np.float32))
    return vectors
...
def train_model(args):
    ...
    trace('making model ...')
    model = EncoderDecoderModel.new(src_size, trg_vocab, args.embed, args.hidden)
    ...
    for epoch in range(args.epoch):
        ...
        gen3 = gens.batch(gens.sorted_parallel(src_vectors, gen2, 100 * args.minibatch), args.minibatch)
